arr = [10, 10, 100, 120, 10, 10, 10, 10, 100, 100, 100, 100, 100, 120, 120, 10, 10, 10, 10, 100, 10, 10, 10, 9, 9]
n = len(arr)
print(arr)
print("n = " + str(n) + "\n")
...
for t in arr_unique:
    ...
    # if sum is 0, set it to 1 to avoid division by 0
    mean_1 = sum([x * (int) (x > t) for x in arr]) / (sum(arr_t) if sum(arr_t) != 0 else 1)
    ...
    variance_1 = sum([((x - mean_1) ** 2) for x in arr if x > t]) / (sum(arr_t) if sum(arr_t) != 0 else 1)
